...
<?php
  ...
  if (isset($_POST['submit']))  { 
    ...
    if (strlen($acc_num) < 8) {
      $_SESSION['error'] = "<p style='color:red;'><i class='fas fa-exclamation-triangle'></i> Account does not exist</p>";
    } else {
      if($count == 1){  
        while($rows=mysqli_fetch_array($ses_sql, MYSQLI_ASSOC)){
          if ($rows['Account'] == $acc_num){
            $_SESSION['acc_num'] = $acc_num;
              ?>
                <script>
                  window.location.href = 'https://beitbridgemun.co.zw/dashboard/';
                </script>
              <?php
          } else {
            $_SESSION['error'] = "<p style='color:red';><i class='fas fa-exclamation-triangle'></i> Account not found in the Database</p>";
          }
        }
      }
      else{  
          $_SESSION['error'] = "<p style='color:red;'><i class='fas fa-exclamation-triangle'></i> Login failed. Invalid Account Number or Password.</p>";  
      }  
  }   
}  
$conn->close; 
                ...
                <div class="form-container">
                  <form action="" method="post" autocomplete="off">
                    <div class="form-control-section pt-3">
                      <input class="form-control" type="text" name= "acc_num" id="account_num" placeholder="Account Number" onkeyup='check()' autocomplete="off" required>
                    </div>

                    <div class="form-control-section mt-4">
                      <input class="form-control" type="password" name="password" id="password" placeholder="Password" onkeyup='check()' autocomplete="off" required>
                    </div>
                    <div class="form-control-section mt-4">
                        <?php 
                            if ($_SESSION['error'] != ""){
                    ...
                    <button name="submit" id='btn-login' class="btn btn-warning" style="width:100%;" onchange='check()' >Login</button>
                    <p class="text mt-4">Don't have an account? <a href="https://www.beitbridgemun.co.zw/register/" class="link-primary text-decoration-none">Register</a></p>
                  </form>
                </div>
              </div>
            </div>
